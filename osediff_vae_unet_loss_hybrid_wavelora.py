...
修改记录 (2026-04-05):
  - WaveAdapter.scale 从普通 float 改为 nn.Parameter(requires_grad=False)
    目的：让 scale 能被 state_dict 保存/恢复，修复 checkpoint 恢复 bug
    scale 不参与梯度更新，保持训练稳定性
...
class WaveAdapter(nn.Module):
    ...
    修改：scale 改为 nn.Parameter(requires_grad=False)
    - 能被 state_dict() 保存/恢复
    - 不参与梯度更新，手动控制值
    - 赋值用 blk.scale.data.fill_(val)
    """
    def __init__(self, channels: int, wave_dim: int = None,
                 res: int = 64, mlp_ratio: float = 1.0, scale: float = 0.2):
        super().__init__()
        self.channels = channels
        # ★ 修改：scale 从普通 float 改为 nn.Parameter(requires_grad=False)
    ...
    def forward(self, x: torch.Tensor):
        ...
        r = self.proj_out(r)
        f = self.ffn(x_norm)
        # ★ self.scale 现在是 nn.Parameter，直接乘即可
