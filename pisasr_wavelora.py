...
修改记录 (2026-04-05):
  - WaveAdapter.scale 现在是 nn.Parameter(requires_grad=False)
  - 所有对 scale 的赋值改为 blk.scale.data.fill_(val)
  - 这样 scale 能被 state_dict 正确保存/恢复
"""
...
class PiSASR(torch.nn.Module):
    ...
    # ── Wave 管理 ──────────────────────────────────────────────────────────
    # ★ 所有 scale 赋值改为 .data.fill_()，因为 scale 现在是 nn.Parameter

    def _freeze_sem_wave(self):
        for blk in self.sem_wave_modules.values():
            blk.requires_grad_(False)
            blk.scale.data.fill_(0.0)  # ★ 改：用 .data.fill_()

    def _unfreeze_sem_wave(self):
        for blk in self.sem_wave_modules.values():
            blk.train()
            blk.requires_grad_(True)
            blk.scale.data.fill_(self._wave_scale_train)  # ★ 改：用 .data.fill_()
            # scale 本身 requires_grad=False，不需要单独设置
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        # 加载 WaveAdapter 权重（包括 scale，因为现在 scale 在 state_dict 里）
    ...
    def save_model(self, outf):
        ...
        # 保存双 WaveAdapter 权重（scale 现在是 nn.Parameter，会自动包含在 state_dict 里）
        pix_sd = {}
...
class PiSASR_eval(nn.Module):
    ...
    def _load_wave_after_merge(self):
        ...
        for key, blk in self.sem_wave_modules.items():
            ...
            if blk_sd: blk.load_state_dict(blk_sd, strict=True)
            blk.eval(); blk.requires_grad_(False)
            # ★ 改：用 .data.fill_()，且 scale 已经从 checkpoint 正确恢复了
            # 这里不需要再手动设置 scale，load_state_dict 已经加载了正确的值
